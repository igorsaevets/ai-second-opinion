...
## agy31pro cannot finish a headless review under a curated allow-list (measured 2026-08-07, round 27)

Three runs of `agy31pro` (Gemini 3.1 Pro, agent profile `deep-researcher`) on one 13.7 KB brief,
three different denied tools, three dead runs:

| run | denied | what it was doing |
|---|---|---|
| 1 | `command(python -c "import re; f=open(...brain/<uuid>...)")` | reading its own brain file |
| 2 | `mcp(cloakbrowser/cloak_launch)` | fetching a bot-protected page |
| 3 | `command(python -c "from bs4 import BeautifulSoup; ...")` | stripping HTML from its own step file |

Run 2 was fixed by adding the cloakbrowser **read path** to `permissions.allow`
(`cloak_launch`/`navigate`/`read_page`/`snapshot`/`close`, deliberately NOT click/type/evaluate/
set_cookies) - consistent with `scrapling/stealthy_fetch` and `firecrawl_scrape` already being
allowed. It got further: **29 tool calls, 24 of them search/fetch**, then died on run 3's command.

🔴 **This is structural, not a missing rule.** The profile shells out to `python -c` with a
different one-off script each time, so an allow-list of exact command strings can never cover it.
The only rules that would are `command(python *)` or `--dangerously-skip-permissions`, and both
grant arbitrary code execution to an unattended agent. Neither is worth one reviewer voice.

🔴 **The harness's own failure message is misleading here and should not be followed.** It says
"Fix it once by running `patch_agy_permissions.py`". That script was already applied and reports
`nothing to do - already patched`; it addresses a *different* failure (the free read-only web
tools), not this one. Do not spend a round re-running it.

**Practical rule: use `agy36flash` for the Gemini-via-subscription voice.** It completed the same
